        ...
        <section id="journey" className="py-24 bg-slate-950 relative overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="relative">
                    ...
                    <div className="hidden md:block absolute right-1/2 top-0 bottom-0 w-1 bg-slate-800 transform translate-x-1/2 rounded-full"></div>
                        ...
                        {JOURNEY_STEPS.map((step, index) => (
                            <div key={step.id} className={`md:flex items-center justify-between gap-8 ${index % 2 === 0 ? '' : 'md:flex-row-reverse'}`}>
                                ...
                                <div className="md:w-[45%] mb-6 md:mb-0">
                                    <Reveal animation={index % 2 === 0 ? 'slide-right' : 'fade-up'} delay={index * 100}>
                                        <div className={`bg-slate-900/50 p-6 rounded-2xl border border-slate-800 hover:border-yellow-500/50 transition-all hover:-translate-y-2 duration-300 group relative overflow-hidden`}>
                                            {/* Glow Effect */}
                                            <div className="absolute -right-10 -top-10 w-32 h-32 bg-yellow-500/5 rounded-full blur-3xl group-hover:bg-yellow-500/10 transition-all"></div>

                                            <div className="flex items-center gap-4 mb-3">
                                                <div className={`w-10 h-10 rounded-lg ${step.color} flex items-center justify-center text-white shadow-lg`}>
                                                    {step.icon}
                                                </div>
                                ...
                                <div className="hidden md:flex w-[10%] justify-center items-center relative">
                                    <div className={`w-12 h-12 rounded-full border-4 border-slate-900 ${step.color} z-10 flex items-center justify-center text-white font-bold`}>
                                        {step.id}
                                    </div>
                                </div>
